<?php
/* 
Pete hacked this file for easier i18n
$form->input will now automatically escape error messages
see line 553 or so
Nov 10 '09
 */
 ...
 */
	function input($fieldName, $options = array()) {
		...
		$options = array_merge($defaults, $options);
		// hacked!
		// add a default to error options, only if error options are not passed or passed as an array.
		$custom_error_options = array('escape' => false);
		if (array_key_exists('error', $options) && is_array($options['error'])) {
			$custom_error_options = array_merge($custom_error_options, $options['error']);
			$options = array_merge($options, array('error' => $custom_error_options));
		}
		elseif (! array_key_exists('error', $options)) {
			$options = array_merge($options, array('error' => $custom_error_options));
		}
